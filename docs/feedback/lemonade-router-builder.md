# Feedback on `lemonade-router-builder`
...
"Generates and validates the JSON only; it does not register it or call the live
...
> 2 to 3 candidates that are either coresident or cheap to swap, and put the
...
one step further: one prompt per rule plus one that should reach `default_model`,
written to `router-prompts.txt`. That turns §2.2's verification step into a loop the
...
Mode B classifier leaf can silently misroute. Those are all settled.
...
not notice (§3.6). That is a question for AMD rather than a patch.
